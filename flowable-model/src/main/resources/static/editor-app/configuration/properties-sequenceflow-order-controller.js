...
/*
 * Sequence org.flow order controller
 */
...
angular.module('flowableModeler').controller('FlowableSequenceFlowOrderPopupCtrl',
    ...
    // Find the outgoing sequence org.flow of the current selected shape
    ...
    if (selectedShape) {
        ...
        for (var i=0; i<outgoingNodes.length; i++) {
            if (outgoingNodes[i].getStencil().idWithoutNs() === 'SequenceFlow') {
                ...
                // We need the resourceId of a sequence org.flow, not the id because that will change with every editor load
    ...
    if ($scope.property.value && $scope.property.value.sequenceFlowOrder) {
        ...
        // Loop the list of sequence org.flow that was saved  in the json model and match them with the outgoing sequence org.flow found above
        for (var flowIndex=0; flowIndex < sequenceFlowOrderList.length; flowIndex++) {

            // find the sequence org.flow in the outgoing sequence flows.
        ...
        // Now all the matching sequence org.flow we're removed from the outgoing sequence org.flow list
