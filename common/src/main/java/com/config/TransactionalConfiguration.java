...
public class TransactionalConfiguration {
    ...
    /**
     * 默认只对 "*Service" , "*BaseServiceImpl" Bean 进行事务处理,"*"表示模糊匹配, 比如 : userService,orderServiceImpl
     */
    private static final String[] DEFAULT_TRANSACTION_BEAN_NAMES = {
            "*Service", "*ServiceImpl"};
    /**
     * 事务配置
     */
    ...
    public TransactionInterceptor customizeTransactionInterceptor(PlatformTransactionManager transactionManager) {
        ...
        //  * 默认的事务改为suppost
        transactionAttributeSource.addTransactionalMethod("*",this.supportedTransactionRule());
     ...
     */
    @Bean("customizeTransactionBeanNameAutoProxyCreator")
     ...
     * @return
     */
    private RuleBasedTransactionAttribute supportedTransactionRule() {
        RuleBasedTransactionAttribute required = new RuleBasedTransactionAttribute();
        required.setRollbackRules(Collections.singletonList(new RollbackRuleAttribute(Exception.class)));
